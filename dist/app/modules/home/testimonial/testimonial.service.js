...
const getAllTestimonial = (filters, paginationOptions) => __awaiter(void 0, void 0, void 0, function* () {
    const { searchTerm } = filters, filtersData = __rest(filters, ["searchTerm"]);
    const andConditions = [];
    if (searchTerm) {
        andConditions.push({
            $or: testimonial_constant_1.testimonialSearchableFields.map((field) => ({
                [field]: {
                    $regex: searchTerm,
                    $options: "i",
                },
    ...
    const result = yield testimonial_model_1.Testimonial.find(whereConditions)
        .sort(sortConditions)
        .skip(skip)
        .limit(limit);
    const total = yield testimonial_model_1.Testimonial.countDocuments();
    return {
        meta: {
            page,
            limit,
...
const getSingleTestimonial = (id) => __awaiter(void 0, void 0, void 0, function* () {
    const result = yield testimonial_model_1.Testimonial.findById({ _id: id }).lean();
    return result;
});
...
const updateTestimonial = (id, payload) => __awaiter(void 0, void 0, void 0, function* () {
    const result = yield testimonial_model_1.Testimonial.findOneAndUpdate({ _id: id }, payload, {
        new: true,
    });
    return result;
});
const deleteTestimonial = (id) => __awaiter(void 0, void 0, void 0, function* () {
    const result = yield testimonial_model_1.Testimonial.findByIdAndDelete({ _id: id });
    return result;
});
